...
class PlaylistModel(QAbstractItemModel):
    ...
    def columnCount(self, parent=QModelIndex()):
        return self.ColumnCount \
            if not parent.isValid() else 0

    def index(self, row, column, parent=QModelIndex()):
        return self.createIndex(row, column) \
            if self.m_playlist is not None and not parent.isValid() and row >= 0 and row < self.m_playlist.mediaCount() and column >= 0 and column < self.ColumnCount else QModelIndex()
